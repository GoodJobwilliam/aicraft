...
- **本轮已完成**: 英文/中文首页首屏收敛到 MCP Code Review；新增 `team-updates.html` / `team-updates.zh.html`；新增预售说明、价格、邮件收集入口和中英文 GitHub team-trial Issue Forms；仓库禁用空白 issue 并提供文档/早期体验联系入口；新增 GitHub Actions 自动回执，申请提交后立即给出安装、试用和升级路径；中英文产品 README 也接入对应试用表单与 10 分钟自助试用包；README、发布指南、市场提交信息和社交草稿同步 Team Updates 口径；新增 `OUTREACH_LOG.md`。
...
- **2026-08-30（收入主线 round 10）**: 新增 `.github/workflows/team-trial-intake.yml`，使用 GitHub 原生 Actions 在 `team-trial` issue 创建后自动回复安装、10 分钟试用、信息安全和升级路径；不依赖外部机器人或付费服务。
