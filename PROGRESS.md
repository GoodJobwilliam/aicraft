...
- 外部触达更新（2026-09-15）：向 `androidAppGuard/asterinas#4` 发布 1 条关于在域知识 MCP 检索前加入确定性本地预检的技术评论（[issue comment](https://github.com/androidAppGuard/asterinas/issues/4#issuecomment-5667259683)）；目前等待维护者回复，仍不能计入试用、付费意向或收入。
...
- 跟进队列增强（2026-09-14）：`scripts/funnel_report.py` 新增 `--as-of YYYY-MM-DD`，根据 `next_follow_up` 输出 due/upcoming 队列；当前真实日志会显示 A9 的 2026-09-20 upcoming 节点，且不改变联系人或收入计数。
- 一键试用命令（2026-09-14）：中英文试用页新增从新目录下载 ZIP、解压并运行 `./run-trial.sh` 的单行路径，减少用户在克隆、文件下载和命令顺序之间的选择成本；根测试 `43 passed`。
