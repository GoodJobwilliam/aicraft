...
- **Pre-commitment**: selects the conditional-start option in the team form (or states the equivalent in email), names a valid target start month in YYYY-MM format, and accepts the selected tier's scope and price, subject to final scope confirmation and checkout. Record `precommitment=yes` only after manually verifying all three pieces.
- **Customer**: payment is confirmed in Creem; a message or intent does not count as revenue. When recording revenue, include a non-secret Creem order or subscription reference in the payment_reference column.
