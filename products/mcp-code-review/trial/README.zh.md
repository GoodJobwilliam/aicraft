...
输出结果后，脚本会打印三个反馈问题和可复制的 GitHub 反馈链接；脚本退出码仍保持审查退出码（本示例为 `1`）。
...
可选的 **Team Rules Pack** 为一次性 `$49`，可[通过 Creem 安全购买](https://www.creem.io/payment/prod_6Z3S3jGNPsCyRSqNi397ZY)。**Team Updates Starter** 是最多 3 人的 `$19/月` 或 `$190/年`；**Team Pilot** 是最多 10 人的 `$99/月` 或 `$990/年`，包含 CI 和调优支持。收费前会先确认范围和开始时间。
